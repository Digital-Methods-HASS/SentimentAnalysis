---
title: "Catharsis or Hygge? The Sentiment of Field Data Capture"
...
# See how this differs from `idig_df`
...
What if we want to get rid of all the numbers (non-text) in `idig_stop`?
```{r skip-numbers}
...
### A word cloud of idig report words (non-numeric)
...
Let's do sentiment analysis on the idig text data using afinn, and nrc. 


### Sentiment analysis with afinn: 

First, bind words in `idig_stop` to `afinn` lexicon:
```{r bind-afinn}
...
Wait, so "excavation" is showing up in NRC lexicon as "negative" and "surprise" and "anticipation"? Let's check:
```{r nrc-lord}
